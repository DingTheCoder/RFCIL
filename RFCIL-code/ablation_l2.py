...
if __name__ == '__main__':
    ...
    #args.num_pretrain = 0  # 用空白模型可以不进行预训练
    ...
    for i in range(4):
        ...
        for iter in trange(10):
            ...
            for idx in idxs_users:
                local = EdgeOpt(args=args, dataset=filtered_dataset_train, idxs=dict_users[idx], user_classes=o_classes[idx])
                #w = local.train_sgd(previous_net=None, global_net=copy.deepcopy(net_glob).to(args.device),net=copy.deepcopy(net_glob).to(args.device),b_vector=best_b,last_round_vector=param_vector,rounds=i)
                #w = local.train(global_net=copy.deepcopy(net_glob).to(args.device),net=copy.deepcopy(net_glob).to(args.device), b_vector=best_b,last_round_vector=param_vector.to(args.device), rounds=i,last_net = copy.deepcopy(last_net).to(args.device))
        ...
        print(f"Test accuracy in {test_classes} is {test_acc}")
