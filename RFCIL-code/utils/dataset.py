...
class DatasetSplit(Dataset):
    def __init__(self, dataset, idxs, target_classes):
    ...
    def __getitem__(self, item):
        ...
        # 如果 label 不在目标类别范围内，重新选择样本，直到找到目标类别的样本
        while label not in self.target_classes:
            item = np.random.choice(self.idxs)
...
def change_label(dataset, selected_classes):
    new_label_mapping = {}
    for i in range(len(selected_classes)):
        new_label_mapping[selected_classes[i]] = i
    dataset_new = [(image, new_label_mapping[label]) for image, label in dataset if label in selected_classes]
    return dataset_new
