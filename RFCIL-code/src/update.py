...
from src.newloss import new_CrossEntropyLoss
...
class EdgeOpt(object):
    def __init__(self, args, dataset=None, idxs=None, user_classes=None):
        ...
        #self.loss_func = new_CrossEntropyLoss()
    ...
    def train(self, global_net, net, b_vector, last_round_vector, rounds, last_net):

        self.args.device = torch.device(
            'cuda:{}'.format(self.args.gpu) if torch.cuda.is_available() and self.args.gpu != -1 else 'cpu')

        last_round_vector.to(self.args.device)
        b_vector = torch.from_numpy(b_vector)  # 将 numpy.ndarray 转换为 Tensor
        # 放到 GPU 上
        b_vector = b_vector.to(self.args.device)
        ...
        for iter in range(local_ep):
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                idx = 0
                for param, last_param in zip(net.parameters(), last_net.parameters()):
                    numel = param.data.numel()
                    b_value = b_vector[idx:idx + numel].reshape(param.data.shape)

                    param.grad += self.args.tradeoff * 2 * (param.data - last_param.data) * b_value
                    idx += numel
    ...
    def old_train(self, previous_net, global_net, net):
        ...
        for iter in range(local_ep):
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if self.args.method == 'moon':
                    ...
                    logits /= 0.5
                    loss += self.args.moon_mu * self.loss_func(logits, torch.zeros(images.size(0)).cuda().long().to(
                        logits.device))
